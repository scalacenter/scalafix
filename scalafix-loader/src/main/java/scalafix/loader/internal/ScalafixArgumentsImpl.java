...
public class ScalafixArgumentsImpl implements ScalafixArguments {
    ...
    public ScalafixArguments withToolClasspath(
        ...
        throw new UnsupportedOperationException("Unsupported method 'withToolClasspath', use withToolDependency*() instead");
    }
    ...
    private ScalafixArguments load() throws ScalafixException {
        ...
        return classLoadScalafixArguments(classLoader)
                .withConfig(config)
                .withToolDependencyCoordinates(toolDependencyCoordinates)
                .withToolDependencyURLs(toolDependencyURLs)
                .withRepositories(repositories)
    ...
    private ScalafixVersions versionsForConfigOrBuiltinScalafixVersion() throws ScalafixException {
        ...
        try {
            ...
            if (typesafeConfig.hasPath(key)) {
                String scalafixVersion = typesafeConfig.getString(key);
                List<URL> versionsJars = ScalafixCoursier.scalafixVersionsJars(
                    repositories,
                    scalafixVersion);
                return ScalafixVersions.get(versionsJars);
            }
        } catch (ConfigException e) {
        }

        return ScalafixVersions.get();
    }
