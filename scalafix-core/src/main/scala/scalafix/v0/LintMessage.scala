 ...
 *
 * @param message
 *   The message to display to the user. If empty, LintID.explanation is used
 *   instead.
 * @param position
 *   Optionally place a caret under a location in a source file. For an empty
 *   position use Position.None.
 * @param category
